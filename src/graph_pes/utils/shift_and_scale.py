...
def guess_per_element_per_atom_mean_and_var(
    per_atom_quantity: torch.Tensor,
    batch: AtomicGraph,
    ...
    # N[structure, Z] is the number of atoms of atomic number Z in structure
    unique_Zs = torch.unique(batch.Z)  # (n_Z,)
    n_atoms, _ = per_atom_quantity.shape
    N = torch.zeros(n_atoms, len(unique_Zs))  # (batch, n_Z)
    Zs_map = {int(Z): i for i, Z in enumerate(unique_Zs)}
    for i, Z in enumerate(batch.Z):
        N[i, Zs_map[int(Z.item())]] = 1.0
...
"""
    ...
    ridge = Ridge(fit_intercept=False, alpha=0.00001)
    ridge.fit(N.numpy(), per_atom_quantity)

    mu_Z = torch.tensor(ridge.coef_.T)

    residuals = per_atom_quantity - N @ mu_Z
    ridge.fit(N.numpy(), residuals**2)

    var_Z = ridge.coef_.T
    var_Z = np.clip(var_Z, min_variance, None)

    means = {int(Z): mu_Z[i] for i, Z in enumerate(unique_Zs)}
    variances = {int(Z): var_Z[i] for i, Z in enumerate(unique_Zs)}
