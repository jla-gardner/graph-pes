...
def default_model():
    ...
    - expansion_features=40 (max_n=3, max_l=3 in original)

    TODO: Confirm with original paper/repo
    """
    return M3GNet(channels=64, layers=3, expansion_features=40, cutoff=CUTOFF)
...
def test_m3gnet_serialization(default_model, tmp_path):
    ...
    loaded_model = M3GNet(cutoff=CUTOFF, channels=64, layers=3, expansion_features=40)
...
def test_m3gnet_bessel_basis(copper_graph):
    ...
    for param in basis_params:
        ...
        logger.info(f"Gradient norm: {param.grad.norm().item():.3e}")
