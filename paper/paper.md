...
Building on the `GraphPESModel` class, we provide independent (re-) implementations of popular MLIP architectures, including `PaiNN` [@Schutt-21-06], `EDDP` [@Pickard-22-07], `NequIP` [@Batzner-22-05], `MACE` [@Batatia-23-01], and `TensorNet` [@Simeon-23-06]. We build upon the `e3nn` [@Geiger-22-07] package for models that act on spherical harmonic representations of the atomic environment.
