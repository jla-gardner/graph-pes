...
These models are trained on labels generated by quantum-mechanical methods, but scale much more favourably with system size, making it possible to simulate the dynamics of large systems (millions of atoms and more) over extended timescales.
...
Building on the `GraphPESModel` class, we provide independent (re-) implementations of popular MLIP architectures, including `PaiNN` [@Schutt-21-06], `EDDP` [@Pickard-22-07], `NequIP` [@Batzner-22-05], `MACE` [@Batatia-22-10], and `TensorNet` [@Simeon-23-06]. We use building blocks provided by the `e3nn` [@Geiger-22-07] package to implement models that act on spherical tensor decompositions.
...
`graph-pes` is beginning to drive a number of projects within our research group, and we hope that it will be useful to many others. In recent preprints, we have described the use of `graph-pes` for fitting NequIP models to datasets created using the `autoplex` software [@Liu-24-12], for assessing zero-shot performance of different graph-network MLIPs [@Mahmoud-25-02], and for fine-tuning and distilling atomistic foundation models [@Gardner-25-06].
